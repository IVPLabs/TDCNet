...
from model.gim.networks.dkm.models.model_zoo.DKMv3 import DKMv3
# from model.gim.networks.lightglue.models.matchers.lightglue import LightGlue
# from model.gim.networks.dkm.models.model_zoo.DKMv3_R18 import DKMv3
# from model.gim.networks.lightglue.superpoint import SuperPoint
# from model.gim.networks.loftr.config import get_cfg_defaults
# from model.gim.networks.loftr.loftr import LoFTR
# from model.gim.networks.loftr.misc import lower_config
# from model.gim.networks.roma.roma import RoMa
from model.gim.tools import get_padding_size
...
def preprocess(image: np.ndarray, grayscale: bool = False, resize_max: int = None,
    ...
    size_new = tuple(map(
        ...
        image.shape[-2:]))
    resize_transform = torchvision.transforms.Resize(size_new)
...
def compute_geom(data,
    ...
    if len(mkpts0) < 8 or len(mkpts1) < 8:
        return {}
...
def wrap_images(img0, img1, geo_info, geom_type):
    img0 = (img0[0].permute((1, 2, 0)).cpu().detach().numpy() * 255)[..., ::-1]
    img1 = (img1[0].permute((1, 2, 0)).cpu().detach().numpy() * 255)[..., ::-1]
    ...
    return rectified_image1.squeeze()
...
class Matcher(nn.Module):
    def __init__(self):
        ...
        self.model = None
        self.detector = None
        if self.args.model == 'gim_dkm':
            self.ckpt = 'gim_dkm_100h.ckpt'
            self.model = DKMv3(weights=None, h=672, w=896)
        # elif self.args.model == 'gim_roma':
        #     self.ckpt = 'gim_roma_100h.ckpt'
        #     self.model = RoMa(img_size=[672])
        # elif self.args.model == 'gim_loftr':
        #     self.ckpt = 'gim_loftr_50h.ckpt'
        #     self.model = LoFTR(lower_config(get_cfg_defaults())['loftr'])
        # elif self.args.model == 'gim_lightglue':
        #     self.ckpt = 'gim_lightglue_100h.ckpt'
        ...
        # weights path
        checkpoints_path = join('model/gim/weights', self.ckpt)
    ...
    def match(self, cur_img, ref_img):
        # 读取三通道图像
        image0 = read_image(cur_img)
        # image0 = cv2.cvtColor(image0, cv2.COLOR_RGB2GRAY)
        image1 = read_image(ref_img)
        ...
        if self.args.model == 'gim_dkm' or self.args.model == 'gim_roma':
            if self.args.model == 'gim_dkm':
            ...
            image0_ = image0_.to(self.device)
            image1_ = image1_.to(self.device)
            ...
            gray0 = gray0.to(self.device)[None]
            gray1 = gray1.to(self.device)[None]
            scale0 = torch.tensor(scale0).to(self.device)[None]
            scale1 = torch.tensor(scale1).to(self.device)[None]
        ...
        # robust fitting
        if kpts0 is not None and kpts1 is not None and len(kpts0) >= 8 and len(kpts1) >= 8:
            ...
            mask = np.zeros(len(kpts0) if kpts0 is not None else 0, dtype=bool)
